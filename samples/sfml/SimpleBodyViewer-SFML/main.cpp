...
public:
    ...
    void processBodies(astra::Frame& frame)
    {
		...
		if (joint.type() == astra::JointType::RightHand )  {
			...
			/*do 
				cin >> tmp;
			while (tmp != "hello" && tmp!="hello\n"); */
			//cout << fixed << joint.world_position().x << "," << fixed << joint.world_position().y << "," << fixed << joint.world_position().z << "\n" << flush;
			fprintf(sockfd_f, "%f,%f,%f\n", joint.world_position().x, joint.world_position().y, joint.world_position().z);
			fflush(sockfd_f);
			//system("sleep 0.167s");
		}
            }
